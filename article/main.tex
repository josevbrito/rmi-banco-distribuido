...
\author[]{Ana Iara Loayza Costa, Higor Pinheiro Costa, \\ José Victor Brito Costa, Milena Freire Britto Neves}
    ...
    \begin{columns}[T]
        \begin{column}{0.50\textwidth}
            \begin{block}{Definição}
                Java RMI permite que um objeto em uma JVM invoque métodos de um objeto localizado em outra JVM, possivelmente em outra máquina.
            \end{block}
...
\begin{frame}{Componentes principais do Java RMI}
    \begin{table}
        ...
        \begin{tabular}{>{\bfseries}p{3.2cm}p{9.2cm}}
            ...
            RemoteException & Indica falhas específicas de comunicação remota. \\
            \bottomrule
        \end{tabular}
    \end{table}
...
\begin{frame}{Arquitetura geral do Java RMI}
    ...
    \begin{tikzpicture}[
        ...
        \node[block] (client) {Aplicação\\cliente};
...
BancoService banco = (BancoService) reg.lookup("BancoService");
    ...
    \vspace{0.2cm}
    \centering
    \small Porta padrão usual do Registry: \code{1099}.
\end{frame}
...
\begin{frame}{Fluxo de uma chamada remota}
    \begin{columns}[T]
        \begin{column}{0.52\textwidth}
            \begin{enumerate}
                \item Cliente executa \code{banco.depositar("cliente", 250.0)}.
                ...
                \item Resultado ou exceção é empacotado.
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Por valor}
                Objetos não remotos são copiados por serialização.
            \end{block}
            \begin{itemize}
...
// callback remoto é passado como stub
\end{lstlisting}
        \end{column}
    \end{columns}
\end{frame}
...
% SLIDE 12 - FALHAS
% =========================================================
\begin{frame}{Riscos adicionais da chamada remota}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \textbf{Uma chamada local pode falhar por:}
            \begin{itemize}
                \item erro de programação;
                \item exceção da regra de negócio;
    ...
    \begin{alertblock}{Ponto-chave}
        Por isso, métodos remotos declaram \code{throws RemoteException}. A interface já avisa que a rede faz parte da chamada.
    \end{alertblock}
\end{frame}
...
% =========================================================
% SLIDE 13.5 - MAPEAMENTO TEORIA x PRÁTICA
% =========================================================
\begin{frame}{O que cada operação do banco demonstra}
    \centering
    \begin{tikzpicture}[
        conceito/.style={rectangle, rounded corners=5pt, draw=rmiTeal, fill=rmiLightGreen, thick, align=center, minimum width=4.2cm, minimum height=0.85cm},
        metodo/.style={rectangle, rounded corners=5pt, draw=rmiBlue, fill=rmiLightBlue, thick, align=center, minimum width=4.2cm, minimum height=0.85cm},
        arr/.style={-{Latex[length=3mm]}, thick, rmiTeal}
    ]
        \node[metodo]   (m1) at (0,  0)    {\code{depositar} / \code{sacar}};
        \node[metodo]   (m2) at (0, -1.4)  {\code{getExtrato}};
        \node[metodo]   (m3) at (0, -2.8)  {\code{registrarListener}};
        \node[metodo]   (m4) at (0, -4.2)  {\code{sacar} (saldo insuficiente)};

        \node[conceito] (c1) at (7.5,  0)    {Passagem de primitivo};
        \node[conceito] (c2) at (7.5, -1.4)  {Passagem por valor\\{\footnotesize objeto serializado}};
        \node[conceito] (c3) at (7.5, -2.8)  {Passagem por referência remota\\{\footnotesize callback servidor → cliente}};
        \node[conceito] (c4) at (7.5, -4.2)  {Exceção que viaja pela rede\\{\footnotesize \code{Serializable}}};
...
\begin{frame}{Passos para implementar Java RMI}
    \begin{columns}[T]
        \begin{column}{0.46\textwidth}
            \begin{block}{No servidor}
                \begin{enumerate}
                    \item Definir interface \code{Remote}
                    \item Implementar com \code{UnicastRemoteObject}
                    \item Criar o \code{Registry} e registrar com \code{rebind}
                \end{enumerate}
            \end{block}
        \end{column}
        \begin{column}{0.46\textwidth}
            \begin{block}{No cliente}
                \begin{enumerate}
                    \setcounter{enumi}{3}
                    \item Localizar o \code{Registry}
                    \item Fazer \code{lookup} e obter o stub
                    \item Invocar métodos normalmente
                \end{enumerate}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{itemize}
    ...
    \item \code{registrarListener} recebe objeto — demonstra \textbf{passagem por referência remota}.
\end{itemize}
\end{frame}
...
ClienteListenerImpl cb =
    new ClienteListenerImpl("cliente");
banco.registrarListener("cliente", cb);
// cb é enviado como stub ao servidor
\end{lstlisting}
            \end{block}
        \end{column}
...
// banco é um stub -- parece local, é remoto
\end{lstlisting}
...
\end{lstlisting}
            ...
            \vspace{0.2cm}
            \centering
            \scriptsize Repositório: \url{https://github.com/josevbrito/rmi-banco-distribuido}
        \end{column}
    \end{columns}
\end{frame}
...
% =========================================================
\begin{frame}{Características Importantes}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Serialização automática}
                \begin{itemize}
                    \item Parâmetros são empacotados pelo RMI.
                    \item Retornos e exceções também trafegam pela rede.
                    \item Objetos por valor precisam ser serializáveis.
                \end{itemize}
            \end{block}
        \end{column}

        \begin{column}{0.48\textwidth}
            \begin{block}{Garbage Collection Distribuído}
                \begin{itemize}
                    \item O RMI acompanha referências remotas.
                    \item Usa uma ideia de \textit{lease} para referências ativas.
                    \item Ajuda a liberar objetos remotos não referenciados.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Segurança e cuidados atuais}
    \begin{itemize}
        \item RMI expõe métodos pela rede; portanto, é necessário controlar quem pode chamá-los.
    ...
    \begin{alertblock}{Mensagem para a apresentação}
        RMI deve ser usado preferencialmente em redes controladas ou sistemas internos, não como API pública aberta na Internet.
    \end{alertblock}
\end{frame}
...
\begin{frame}{Limitações e desafios}
    \begin{table}
        ...
        \begin{tabular}{>{\bfseries}p{3.4cm}p{4.5cm}p{4.5cm}}
            \toprule
            Desafio & Problema & Cuidado prático \\
            \midrule
            Latência & Chamadas remotas são mais lentas que chamadas locais. & Evitar chamadas muito finas; agrupar operações. \\
            Falhas parciais & Cliente, servidor ou rede podem falhar separadamente. & Tratar \code{RemoteException}, timeouts e recuperação. \\
            Acoplamento & Cliente e servidor compartilham interfaces Java. & Manter contratos estáveis e versionados. \\
            Serialização & Mudanças em classes podem quebrar compatibilidade. & Usar \code{serialVersionUID} e evolução cuidadosa. \\
            Firewall/NAT & Objetos exportados sem porta fixa podem dificultar configuração. & Definir portas e políticas de rede. \\
            \bottomrule
        \end{tabular}
    \end{table}
\end{frame}
...
\begin{frame}{RMI vs outras tecnologias}
    \begin{table}
        ...
        \begin{tabular}{>{\bfseries}p{2.2cm}p{4.7cm}p{5.3cm}}
            ...
            \midrule
            Java RMI & Ambiente 100\% Java, rede controlada, sistemas internos ou legados. & Modelo orientado a objetos e forte integração com Java. \\
            REST/HTTP & APIs web, integração entre linguagens e serviços públicos. & Mais comum em sistemas modernos e interoperáveis. \\
            gRPC & Alto desempenho, contratos bem definidos e múltiplas linguagens. & RPC moderno com Protocol Buffers. \\
    ...
    \begin{block}{Leitura crítica}
        RMI não é necessariamente ``melhor'' ou ``pior''. Ele faz sentido quando o contexto é Java, interno e orientado a objetos.
    \end{block}
\end{frame}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            ...
            \begin{itemize}
                ...
                \item Aplicações legadas que já usam RMI.
                \item Prototipação e ensino de sistemas distribuídos.
                \item Comunicação entre componentes Java em rede controlada.
            \end{itemize}
        \end{column}
...
\begin{frame}{Conclusão}
    \begin{block}{Ideia principal}
        Java RMI é um estudo de caso clássico de invocação remota orientada a objetos.
    \end{block}
    ...
    \begin{itemize}
        ...
        \item \code{RemoteException} lembra que chamada remota não é chamada local.
    \end{itemize}

    \vspace{0.2cm}
    \begin{alertblock}{Mensagem final}
        RMI facilita a programação distribuída em Java, mas não elimina os desafios fundamentais: latência, falhas, acoplamento, versionamento e segurança.
    \end{alertblock}
\end{frame}
...
% =========================================================
% SLIDE 27 - Agradecimento
% =========================================================
\begin{frame}
    \centering
    \vspace{1cm}
    {\Huge Agradecemos pela atenção!}
\end{frame}
